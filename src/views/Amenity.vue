    ...
    <div :style="{backgroundImage: 'url(/src/assets/sidebarimg.jpeg)'}" class="d-flex flex-column p-3 text-white bg-dark" id="sidenav" style="height: 100vh;  position: -webkit-sticky;position: sticky;top: 0;"> 
        ...
        <ul class="nav nav-pills flex-column mb-auto">
          <li>
            ...
            <li>
                <a href="/amenity" class="nav-link active">
                    <svg class="bi me-2" width="16" height="16">
            ...
            <div class="row py-lg-4">
              <div class="col-lg-6 col-md-8 mx-auto">
                ...
                <tbody>
                    ...
                    <tr v-for="item in searchAmenity" v-bind:key="item.id">
                      <td class ="text-left">
                      <div class="col">
                        <div class="card shadow-sm" style="max-width: 325px">   
                          <a class="lightbox"><img v-bind:src="item.ameimageUrl"  class="card-img-top" style="width: 100%; max-width: 400px; height:200px;"></a>
                            <!-- <img src="https://i.ibb.co/Qdz72J6/amenity.jpg" alt="jerry" style="width:350px auto;height:150px;"> -->
                          <div class="card-body" style="height: 190px">
                            <h6 >{{item.amename}}</h6> 
                            <p>{{item.ameloca}}</p>
                            <p>{{item.amecat}}</p>
                            <router-link :to="`/amenitydetail/${item.id}`">
...
export default {
  ...
  data() {
    return {
     ...
     created () {
    db.collection("amenities").orderBy("amename","asc").get().then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
        this.amenity.push({ id:doc.id,...doc.data()})
        console.log(this.amenity)
        });
    });
    },
    computed:{
    searchAmenity: function(){
      return this.amenity.filter((amen)=>{
        return amen.amename.match(this.search)
      })
    }
   },
   mounted() {
